...
class MultiplePaymentGatewaysTest extends TestCase
{
    ...
    public function test_can_be_paid_credit_card_by_cielo(): void
    {
        ...
        $this->assertEquals(201, $paymentReturn->getStatusCode());
    }
    ...
    public function test_can_be_paid_credit_card_by_fpay(): void
    {
        ...
        $paymentReturn = $this->payment->payCreditCard($this->fpay, $this->order->merchantOrderId,
            $this->customer, 50.00, $this->creditCard, 1, 'AV');

        $this->assertEquals(200, $paymentReturn->getStatusCode());
    }

    public function test_can_be_get_payment_cielo()
    {
        $paymentReturn = $this->payment->payCreditCard($this->cielo, $this->order->merchantOrderId, $this->customer,
            50.000, $this->creditCard, 1, 'ByMerchant')->getBody();

        $paymentId = json_decode($paymentReturn, true)["Payment"]["PaymentId"];

        $paymentGatted = $this->payment->getTransaction($this->cielo, $paymentId)->getBody()->getContents();

        $this->assertStringContainsString("AuthorizationCode",$paymentGatted);
    }
    ...
    public function test_can_be_get_payment_fpay()
    {
        ...
        $paymentReturn = $this->payment->payCreditCard($this->fpay, $this->order->merchantOrderId,
            $this->customer, 50.00, $this->creditCard, 1, 'AV')->getBody();

        $paymentId = json_decode($paymentReturn, true)["data"]["fid"];

        $paymentGatted =  $this->payment->getTransaction($this->fpay, $paymentId)->getBody()->getContents();
    ...
    public function test_can_be_reverse_payment_cielo()
    {
        ...
        $paymentReversed =  $this->payment->reverseTransaction('PUT', $this->cielo, $paymentId);
    ...
    public function test_can_be_reverse_payment_fpay()
    {
        ...
        $paymentReversed =  $this->payment->reverseTransaction('DELETE', $this->fpay, $paymentId);
